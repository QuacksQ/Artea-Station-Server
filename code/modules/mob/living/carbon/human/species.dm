...
/datum/species/proc/regenerate_organs(mob/living/carbon/C, datum/species/old_species, replace_current = TRUE, list/excluded_zones, visual_only = FALSE)
	...
	for(var/obj/item/organ/external_organ in C.cosmetic_organs)
...
/datum/species/proc/handle_body(mob/living/carbon/human/species_human)
	...
	if(noggin && !(HAS_TRAIT(species_human, TRAIT_HUSK)))
		...
		if(!(NOEYESPRITES in species_traits))
			...
			if(!eye_organ)
				no_eyeslay = mutable_appearance('icons/mob/species/human/human_face.dmi', "eyes_missing_both", -BODY_LAYER)
...
/datum/species/proc/randomize_external_organs(mob/living/carbon/human/human_mob)
	for(var/obj/item/organ/organ_path as anything in cosmetic_organs)
		var/obj/item/organ/randomized_organ = human_mob.getorgan(organ_path)
		if(randomized_organ)
...
/datum/species/proc/spec_death(gibbed, mob/living/carbon/human/human)
	for(var/obj/item/organ/organ in human.processing_organs)
		organ.owner_death()
...
/datum/species/proc/spec_attacked_by(obj/item/weapon, mob/living/user, obj/item/bodypart/affecting, mob/living/carbon/human/human)
	...
	apply_damage(weapon.force * weakness, weapon.damtype, def_zone, armor_block, human, sharpness = weapon.get_sharpness(), attack_direction = attack_direction, attacking_item = weapon)
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE, sharpness = NONE, attack_direction = null, attacking_item)
	SEND_SIGNAL(H, COMSIG_MOB_APPLY_DAMAGE, damage, damagetype, def_zone, blocked, sharpness, attack_direction, attacking_item)
	...
	switch(damagetype)
		if(BRUTE)
			...
			if(BP)
				if(BP.receive_damage(damage_amount, 0, sharpness = sharpness, damage_source = attacking_item))
		...
		if(BURN)
			...
			if(BP)
				if(BP.receive_damage(0, damage_amount, sharpness = sharpness, damage_source = attacking_item))
					H.update_damage_overlays()
			else
