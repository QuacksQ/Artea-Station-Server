/mob/living/carbon/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked = FALSE, forced = FALSE, spread_damage = FALSE, sharpness = NONE, attack_direction = null, attacking_item)
 ...
 * It automatically updates health status
 */
/mob/living/carbon/take_bodypart_damage(brute = 0, burn = 0, updating_health = TRUE, required_bodytype, check_armor = FALSE, sharpness = NONE)
	var/list/obj/item/bodypart/parts = get_damageable_bodyparts(required_bodytype)
	if(!parts.len)
		return
	var/obj/item/bodypart/picked = pick(parts)
	if(picked.receive_damage(brute, burn, check_armor ? run_armor_check(picked, (brute ? MELEE : burn ? FIRE : null)) : FALSE, sharpness = sharpness))
		update_damage_overlays()
...
/mob/living/carbon/take_overall_damage(brute = 0, burn = 0, updating_health = TRUE, required_bodytype)
	...
	while(parts.len && (brute > 0 || burn > 0))
		...
		update |= picked.receive_damage(brute_per_part, burn_per_part, FALSE, updating_health, required_bodytype) // disabling wounds from these for now cuz your entire body snapping cause your heart stopped would suck
