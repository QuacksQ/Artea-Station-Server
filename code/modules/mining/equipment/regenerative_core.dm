...
/obj/item/hivelordstabilizer/afterattack(obj/item/organ/target_organ, mob/user, proximity)
	...
	if(!proximity)
		return
	var/obj/item/organ/regenerative_core/target_core = target_organ
	if(!istype(target_core, /obj/item/organ/regenerative_core))
...
/obj/item/organ/regenerative_core/Insert(mob/living/carbon/target_carbon, special = 0, drop_if_replaced = TRUE)
	. = ..()
	if(!.)
		return
...
/obj/item/organ/regenerative_core/Remove(mob/living/carbon/target_carbon, special = 0)
	if(!inert && !special)
